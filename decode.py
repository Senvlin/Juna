"""解码模块
感谢x1uc编写的(Python版本)[https://github.com/yihong0618/shanbay_remember/blob/main/Decoder.py]
"""
...
class Decoder:
    ...
    def decode(enc) -> dict | str:
        ...
        return json.loads(base64.b64decode(raw_encode).decode("utf-8"))
...
class Random:
    ...
    def next_state(self):
        y = self.status[3]
        x = Num.xor(
            Num.and_(self.status[0], Random.BAY_MASK),
            Num.xor(self.status[1], self.status[2]),
        )
        ...
        self.status = [
            self.status[1],
            self.status[2],
            Num.xor(x, Num.shift_left(y, Random.BAY_SH1)),
            y,
        ]
...
class Tree:
    ...
    def add_symbol(self, ch, length):
        ...
        for _ in range(length):
            ...
            while (
                inner_char in ptr.get_children()
                and ptr.get_children()[inner_char].get_char() != "."
            ):
                inner_char = Tree.B32_CODE[self.random.generate(32)]
